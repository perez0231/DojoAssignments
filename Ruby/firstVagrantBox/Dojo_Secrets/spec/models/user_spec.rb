...
RSpec.describe User, type: :model do
  context "with valid attributes" do
    ...
    it "automattically encrypts the password into the password_digest attribute" do
      expect(build(:user).password_digest.present?).to eq(true)
    end
    it 'email as lowercase' do
      expect(create(:user, email:'DP@GMAIL.COM').email_downcase).to eq('dp@gmail.com')
    end
    end
    context "with invalid attributes and it should not save" do
      it 'name is blank' do
